...
router.delete("/:uuid", auth, async function (req, res) {
  try {
    ...
    let item = await db["services"].findOne({ raw: true, where: { uuid } });
    if (item) {
      const params = {
        Bucket: bucket_name,
        Key: item.uuid,
      ...
      await db["services"].destroy({ where: { uuid } });
    ...
    if (item) {
      try {
        if (Object.entries(req.body).length > 0) {
          ...
          await db["services"].update({ ...item }, { where: { uuid } });
        }
      } catch (err) {
...
router.get("/all", async function (req, res) {
  try {
    ...
    let response = await db["services"].findAll(queryOptions);
...
router.get("/:uuid", async function (req, res) {
  try {
    ...
    let response = await db["services"].findOne(queryOptions);
    let objectParams, objectCommand, urlToS3;
    objectParams = {
      Bucket: bucket_name,
      Key: response.uuid,
